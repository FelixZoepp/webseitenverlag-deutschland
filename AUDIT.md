...
| R5 | **Upsell-Aktivierung** `lib/upsell-orchestrator.ts` | Hängt an `lib/payment.ts`-Stub (No-Op-Provider); Annahme aktiviert faktisch nichts Abrechenbares | Phase E/F: Orchestrator auf Stripe-Subscription-Items umstellen, `payment.ts`-Stub fällt weg. **Stand Ende Phase G:** neuer Kaufweg (upsell_orders/contracts/Stripe-Checkout) ist live; der alte Aktivierungs-Flow (Orchestrator + rechnungs_posten + Upsells-Tab-Kacheln in der Kundendetailseite) läuft parallel weiter → Ablösung/`_legacy` in Phase H |
