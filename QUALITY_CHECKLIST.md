...
Stand: 42 Regeln · Layer: Config (Generierung) → Render (HTML) → Browser (Playwright)
...
| `R-GO-LIVE` | Freischaltung setzt noindex=false + status=published + Subdomain/Domain. Subdomain ist DNS-konform (≤63 Zeichen, a-z/0-9/-). | Render | Kein Autofix — Freischalt-Logik in der API-Route prüfen. | `app/api/admin/sites/[siteId]/freischalten/route.ts` | `scripts/test-go-live.ts` |
