...
- [ ] **~246 Higgsfield-Credits freigeben** für die Solarflow-Asset-Kette (Rezeptliste: `rezepte/REZEPTE_SCRUB_PV.md`): 1× Entry Still (gpt_image_2, 2k, 7 Credits) + 5× seedance_2_0-Videos (5 s, 1080p, 225 Credits) + Reserve 14. **Sequentiell zwingend:** letzter Frame von Video N = start_image von Video N+1 (Seam-Locking) — nie parallel generieren. Nachbearbeitung braucht lokal `ffmpeg` (Seam-Frames, Poster, Encoding, Frame-Sequenz ≤ 5 MB für den Canvas-Modus). Bis zur Freigabe rendert die Komposition automatisch den statischen Poster-Modus.
