...
- [ ] **Vercel Env-Vars setzen**: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` (fehlen noch; `SUPABASE_SERVICE_ROLE_KEY` ist bereits gesetzt)
...
- [x] **Higgsfield-Assets galabau generieren** ✅ Felix 2026-07-22 („gib galabau frei") — 21/21 Slots generiert + importiert + approved (58,25 Credits, Details: PROGRESS.md „GaLaBau-Asset-Lauf"). Hinweis: `SUPABASE_SERVICE_ROLE_KEY` in Vercel (Production) endet auf ein literales `\n` — bei erneutem `vercel env pull` strippen oder Secret einmal neu setzen
...
- [x] **Maler B5 Optik-Freigabe** ✅ Felix 2026-07-22 („gib alle frei") — `frozen:true` gesetzt; B6 (echte Assets) hängt weiter am Higgsfield-Key
- [ ] **Higgsfield-Assets maler generieren** (Rezeptliste: `rezepte/REZEPTE_MALER.md`): Kette Hero → Video → BA-Paar (NACHHER zuerst!) → Galerie; Fertig-Kriterium ≥25 approved inkl. 1 Paar, 1 Hero, 1 Video, 4 Galerie-Bilder (mind. 2 Kategorien). Import: `npx tsx scripts/import-assets.ts --branche maler --dir <download-ordner> --approve`
...
## Premium Scroll-Scrub (`scrub-story-v1`, PV-Seed Solarflow)
- [x] **~246 Higgsfield-Credits freigeben** ✅ Felix 2026-07-22 („gib alle credits frei, generiere die assets") — Kette komplett gelaufen (232 Credits verbraucht, Rezept `rezepte/REZEPTE_SCRUB_PV.md` 1:1 inkl. Seam-Locking), 303 WebP-Frames (4,87 MB) + 5 Poster unter `public/media/pv/`, `inhalte.frames` gesetzt ⇒ **Scrub-Modus aktiv** (Details: PROGRESS.md „Scrub-PV Asset-Lauf")
- [x] **Premium-Einordnung entschieden** ✅ Felix 2026-07-22: Premium-Videos (Scroll-Scrub) sind im höchsten Paket **Premium (growth) 249 €/Monat** enthalten — KEINE 4. Stufe. `config/plans.ts` bildet das bereits ab (`scroll_story` im Growth-Editor, `preis_hinweis` 249 € netto), keine Code-Änderung nötig. PV ist **eigene Branche 17** in der Matrix (TEMPLATE_FABRIK_MASTER §3 + PROGRESS.md-Matrix). Offen bleibt nur die Stripe-Preis-ID fürs Premium-Paket (Stripe-Keys, siehe §Stripe).
- [x] **Seed-Musterdaten bestätigt** ✅ Felix 2026-07-22: Solarflow/Osnabrück-Musterdaten und PV-Kontakt-Placeholder ok — bleiben wie sie sind.
