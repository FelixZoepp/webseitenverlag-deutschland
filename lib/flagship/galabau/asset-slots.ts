/**
 * Auftrag T1.5 — Asset-Slots der Komposition „galabau-landing-v1".
 *
 * Registrierungs-Gate: validiereAssetSlots wirft beim Modul-Load bei
 * invalider Deklaration (galabau/render.ts importiert dieses Modul).
 *
 * Slot-Keys intern snake_case (Schema-Regel); die Higgsfield-Dateinamen
 * sind dieselben IDs mit Bindestrich (hero-bg.jpg → hero_bg).
 * Paar-Kopplung: ba_before (vorher) → ba_after (nachher), IMMER via pair_id.
 */
import { validiereAssetSlots, type AssetSlots } from '@/lib/assets/slots'

export const GALABAU_ASSET_SLOTS: AssetSlots = validiereAssetSlots('galabau-landing-v1', {
  // Hero: 16:9, min 1920 — Video optional (Growth), Fallback aufs Hero-Bild
...
export function slotKeyAusDateiname(dateiname: string): string | null {
  const basis = dateiname.replace(/\.[a-z0-9]+$/i, '').toLowerCase().replace(/-/g, '_')
  return basis in GALABAU_ASSET_SLOTS ? basis : null
}
