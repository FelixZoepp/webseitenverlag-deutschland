...
function logoHtml(nav: NavInhalt, hell: boolean): string {
  const bild = (nav as unknown as Record<string, unknown>).logo_bild as string | undefined
  if (bild) {
...
<header class="vhero" id="top">
  ${poster
    ? `<img class="vhero-img" src="${escAttr(poster)}" alt="${escAttr(hero.media.alt || hero.media.label)}" width="1440" height="900" loading="eager">`
    : `<div class="vhero-img media" data-label="${escAttr(hero.media.label)}"></div>`}
  <div class="vshade"></div>
  <div class="wrap vinner">
    <div class="rv in" style="max-width:640px">
      <span class="eyebrow">${esc(hero.eyebrow)}</span>
