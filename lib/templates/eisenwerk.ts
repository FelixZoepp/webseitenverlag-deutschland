// Eisenwerk Template — Fitness/Sport/Gym
// Dark, athletic design with accent color highlights

export interface EisenwerkConfig {
  businessName: string
  tagline: string
  heroHeadline: string
  heroAccent: string
...
export function renderEisenwerkTemplate(config: EisenwerkConfig, siteId?: string): string {
  const c = config
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://webseitenverlag-deutschland.vercel.app'

  // Derived color values from config
  const primary = c.colors.primary || '#0a0a0a'
  const accent = c.colors.secondary || '#d9f55b'
  const bg = c.colors.background || '#f5f5f4'
  const textColor = c.colors.text || '#f5f5f4'

  const primary2 = lighten(primary, 10)
  const primary3 = lighten(primary, 18)
  const steel = lighten(primary, 32)
  const accentHover = darken(accent, 20)
  const accentSoft = hexToRgba(accent, 0.18)
  const accentGlow = hexToRgba(accent, 0.5)
  const textSoft = hexToRgba(textColor === '#f5f5f4' ? textColor : bg, 0.72)
  const textDim = hexToRgba(textColor === '#f5f5f4' ? textColor : bg, 0.45)
  const borderColor = hexToRgba(textColor === '#f5f5f4' ? textColor : bg, 0.08)
  ...
  const heroVisualStyle = c.heroImageUrl
    ...
    : `background: linear-gradient(180deg, transparent 50%, rgba(0,0,0,0.7)), linear-gradient(135deg, #3a3a3a 0%, #1a1a1a 60%, #0a0a0a 100%);`

  const heroVisualAfter = c.heroImageUrl
    ? ''
    : `content: 'IMG: Athlet:in beim Training'; position: absolute; bottom: 24px; left: 24px; font-family: var(--font-mono); font-size: 11px; color: var(--white-dim); letter-spacing: 0.08em;`
...
<section id="schedule" class="schedule">
  <div class="container">
    <div class="section-head">
      ...
      <h2 class="display">Unser <em>Kursplan</em></h2>
      <p>Live-Buchung. Wenn ein Platz frei ist, ist er buchbar.</p>
    </div>
...
<section id="programs" class="programs">
  <div class="container">
    <div class="section-head">
      <span class="eyebrow">Trainings-Programme</span>
      <h2 class="display">Unsere <em>Programme</em></h2>
      <p>F\u00fcr jedes Level das richtige Training.</p>
    </div>
...
<section id="pricing" class="pricing">
  <div class="container">
    <div class="section-head">
      ...
      <h2 class="display">Unsere <em>Preise</em></h2>
...
<section id="trainers" class="trainers">
  <div class="container">
    <div class="section-head">
      ...
      <h2 class="display">Unsere <em>Trainer</em></h2>
      <p>Erfahrene Coaches mit echtem Hintergrund.</p>
    </div>
    <div class="trainers-grid">
      ${c.trainers.map((t, i) => `<div class="trainer-card trainer-${i + 1}">
        <div class="trainer-img" ${t.imageUrl ? `style="background: url('${esc(t.imageUrl)}') center/cover no-repeat;"` : `style="background: linear-gradient(160deg, ${lighten(primary, 20 + i * 10)}, ${primary});"`}></div>
  ...
  <div class="container location-grid">
    ...
    <div class="location-map">
      <svg class="map-svg" viewBox="0 0 600 460" preserveAspectRatio="xMidYMid slice">
        ...
        <rect class="map-block" x="190" y="30"  width="80"  height="100" rx="4"/>
        <rect class="map-block" x="300" y="40"  width="140" height="60"  rx="4"/>
        <rect class="map-block" x="470" y="50"  width="100" height="120" rx="4"/>
        <rect class="map-block" x="50"  y="160" width="100" height="100" rx="4"/>
        <rect class="map-block" x="180" y="170" width="100" height="80"  rx="4"/>
        <rect class="map-block" x="320" y="180" width="120" height="100" rx="4"/>
        <rect class="map-block" x="470" y="210" width="100" height="80"  rx="4"/>
        <rect class="map-block" x="40"  y="300" width="130" height="100" rx="4"/>
        <rect class="map-block" x="200" y="290" width="100" height="120" rx="4"/>
        <rect class="map-block" x="330" y="320" width="110" height="90"  rx="4"/>
        <rect class="map-block" x="470" y="330" width="100" height="100" rx="4"/>
...
<style>
  ...
  :root {
    ...
    --shadow-image: 0 20px 60px rgba(0, 0, 0, 0.6);
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .display em { font-style: italic; color: var(--volt); font-variation-settings: "opsz" 144, "SOFT" 80; }
  ...
  nav {
    ...
    background: ${hexToRgba(primary, 0.88)};
  ...
  .nav-links a { transition: color 0.2s; color: var(--white-soft); }
  ...
  .nav-cta:hover { background: var(--volt-hover); transform: translateY(-1px); }
  ...
  .hero::before {
    ...
    inset: 0;
    background-image:
      linear-gradient(90deg, ${hexToRgba(accent, 0.02)} 1px, transparent 1px),
      linear-gradient(0deg, ${hexToRgba(accent, 0.02)} 1px, transparent 1px);
  ...
  .hero-tag {
    ...
    border: 1px solid var(--volt-soft);
    color: var(--volt);
    padding: 8px 16px;
    border-radius: 999px;
    font-family: var(--font-mono);
  ...
  @keyframes pulse {
    0%, 100% { opacity: 1; box-shadow: 0 0 0 0 var(--volt-glow); }
  ...
  .btn-primary:hover { background: var(--volt-hover); transform: translateY(-2px); box-shadow: 0 12px 32px ${hexToRgba(accent, 0.5)}; }
  ...
  .live-badge {
    position: absolute;
    top: 24px; right: 24px;
    background: ${hexToRgba(primary, 0.85)};
  ...
  /* ========================================
     PROGRAMS
     ======================================== */
  .programs { background: var(--black-2); }
  .programs-grid {
    display: grid;
  ...
  .program-card:hover { border-color: var(--border-hover); transform: translateY(-4px); }
  .program-icon {
  ...
  /* ========================================
     SCHEDULE WIDGET
     ======================================== */
  .schedule { background: var(--black-2); }
  .schedule-tabs {
    display: flex;
    gap: 0;
    border-bottom: 1px solid var(--border);
  ...
  .class-row:hover { border-color: var(--border-hover); transform: translateX(4px); }
  .class-time {
    font-family: var(--font-display);
    font-weight: 600;
    font-size: 22px;
  ...
  .class-book:hover { background: var(--volt); }
  ...
  .price-card.featured {
    ...
    background: linear-gradient(180deg, ${hexToRgba(accent, 0.05)}, var(--black-3));
  }
  ...
  .price-cta:hover { background: var(--volt); color: var(--black); border-color: var(--volt); }

  /* ========================================
     TRAINER PROFILE
     ======================================== */
  .trainers { background: var(--black-2); }
  .trainers-grid {
    display: grid;
  ...
  .trainer-overlay {
    ...
    padding: 24px;
    background: linear-gradient(180deg, transparent, ${hexToRgba(primary, 0.92)});
  }
  .trainer-name {
    font-family: var(--font-display);
    font-weight: 600;
  ...
  .testi-rating svg { width: 16px; height: 16px; color: var(--volt); fill: var(--volt); }
  .testi-quote {
    font-family: var(--font-display);
    font-weight: 500;
    font-size: 18px;
  ...
  /* ========================================
     LOCATION + MAP
     ======================================== */
  .location { background: var(--black-2); }
  .location-grid {
    display: grid;
  ...
  .location-detail .value strong { color: var(--volt); font-weight: 600; }
  ...
  .map-street { stroke: ${hexToRgba(bg, 0.08)}; stroke-width: 2; fill: none; }
  .map-street-major { stroke: ${hexToRgba(bg, 0.14)}; stroke-width: 4; fill: none; stroke-linecap: round; }
  .map-block { fill: ${hexToRgba(bg, 0.04)}; }
  .map-water { fill: ${hexToRgba(accent, 0.04)}; }
  .map-pin {
    position: absolute;
    top: 50%; left: 50%;
  ...
  .map-pin-circle {
    ...
    justify-content: center;
    color: var(--black);
    box-shadow: 0 8px 24px ${hexToRgba(accent, 0.4)};
  ...
  /* ========================================
     FINAL CTA
     ======================================== */
  .final-cta { background: var(--black); padding: 120px 0; }
  .final-cta-inner {
    background: linear-gradient(135deg, var(--volt), ${darken(accent, 30)});
    color: var(--black);
    border-radius: 32px;
    padding: 80px 64px;
    text-align: center;
  ...
  .final-cta-inner .btn:hover { background: var(--black-3); transform: translateY(-2px); }
  ...
  .cta-form input:focus, .cta-form textarea:focus {
    border-color: ${hexToRgba(primary, 0.5)};
    background: ${hexToRgba(primary, 0.14)};
  }
  .cta-form input::placeholder, .cta-form textarea::placeholder { color: ${hexToRgba(primary, 0.45)}; }
  ...
  <div class="container hero-grid">
    ...
    <div class="hero-visual">
      <div class="live-badge">
        <div class="live-label"><span class="live-dot"></span>Gerade live</div>
        <div class="live-count" id="liveCount">47</div>
        <div class="live-sub">Aktive Mitglieder</div>
      </div>
    </div>
  </div>
</section>
...
<section id="cta" class="final-cta">
  <div class="container">
    <div class="final-cta-inner">
      <h2 class="display">${esc(c.ctaText)}</h2>
      <p>Schreib uns eine Nachricht und wir melden uns bei dir.</p>
      <form class="cta-form" onsubmit="return false">
        ...
        <button class="btn" type="button" id="cta-submit" style="white-space:nowrap;width:auto;background:var(--black);color:var(--volt)">${esc(c.ctaText)} &rarr;</button>
      </form>
    </div>
  </div>
</section>
  ...
  <div class="container">
    <div class="footer-grid">
      ...
      <div class="footer-col">
        <h4>Info</h4>
        <ul>
          ${c.trainers.length > 0 ? '<li><a href="#trainers">Trainer</a></li>' : ''}
...
<script>
  ...
  (function liveCounter() {
    ...
    setInterval(function() {
      ...
      current = Math.max(35, Math.min(62, current + delta));
  ...
  ${siteId ? `(function contactForm() {
    ...
    ctaBtn.addEventListener('click', async function() {
      ...
      try {
        ...
        var d = await r.json();
        if (d.success) {
          form.innerHTML = '<div style="text-align:center;padding:40px"><h3 style="font-size:1.6rem;margin-bottom:12px">Vielen Dank!</h3><p style="font-size:1.05rem;opacity:0.7">Wir melden uns schnellstm\\u00f6glich bei dir.</p></div>';
        } else {
...
<!-- Cookie Banner -->
<div id="cookie-banner" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:9999;background:${hexToRgba(primary, 0.97)};backdrop-filter:blur(12px);color:var(--white);padding:16px 24px;font-size:.85rem;line-height:1.6">
  <div style="max-width:1100px;margin:0 auto;display:flex;align-items:center;gap:20px;flex-wrap:wrap">
    <p style="flex:1;min-width:200px;margin:0;color:var(--white-soft)">Diese Website verwendet technisch notwendige Cookies. Weitere Informationen findest du in unserer <a href="${esc(c.privacyUrl || '#')}" style="color:var(--white);text-decoration:underline">Datenschutzerkl&auml;rung</a>.</p>
    <button onclick="document.getElementById('cookie-banner').style.display='none';localStorage.setItem('cookies_accepted','1')" style="background:var(--volt);color:var(--black);border:none;padding:8px 20px;border-radius:50px;font-weight:600;font-size:.82rem;cursor:pointer;font-family:inherit">Verstanden</button>
  </div>
</div>
