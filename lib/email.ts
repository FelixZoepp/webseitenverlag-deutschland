...
/**
 * Cloudflare-Token widerrufen/abgelaufen (Phase G): Site wurde automatisch auf
 * multi_tenant-Auslieferung umgestellt — Kunde soll den Zugang neu verbinden.
 */
...
export async function sendSeoFreigabeEmail(
  toEmail: string,
  customerName: string,
  keyword: string,
  siteId: string
): Promise<{ success: boolean; error?: string }> {
  const freigabeUrl = `${APP_URL}/dashboard/${siteId}/seo`
  const text = `deine neue SEO-Unterseite f\u00fcr das Keyword \u201e${keyword}\u201c ist fertig. Sieh sie dir kurz an und schalte sie mit einem Klick frei \u2014 erst dann geht sie online. Im Dashboard findest du auch den technischen Check der Seite.`
  try {
    await getResend().emails.send({
      from: `${FROM_NAME} <${FROM_EMAIL}>`,
      to: toEmail,
      subject: `Neue SEO-Seite wartet auf deine Freigabe: ${keyword}`,
      html: `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f3f4f6"><div style="max-width:600px;margin:0 auto;padding:24px"><div style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="background:#1f2937;padding:24px 32px"><h1 style="color:#fff;font-size:18px;margin:0">Webseitenverlag Deutschland</h1></div><div style="padding:32px"><h2 style="font-size:20px;color:#111827;margin:0 0 16px">Neue SEO-Seite ist fertig</h2><p style="color:#374151;line-height:1.6">Hallo ${customerName},</p><p style="color:#374151;line-height:1.6">${text}</p><div style="margin-top:24px;text-align:center"><a href="${freigabeUrl}" style="display:inline-block;background:#2563eb;color:#fff;padding:12px 24px;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none">Seite ansehen &amp; freigeben</a></div></div></div></div></body></html>`,
      text: `Hallo ${customerName},\n\n${text}\n\nFreigeben: ${freigabeUrl}`,
    })
    return { success: true }
  } catch (err: unknown) {
