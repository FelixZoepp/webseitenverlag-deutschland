...
- szenen.hero: CLOSE-UP auf das HANDWERK/ERGEBNIS — die Arbeit zeigen, NICHT die Person (DSGVO). Hauptgeschehen auf der RECHTEN Bildhälfte (Text-Overlay liegt links). Konkret: Reinigung → Hand mit Abzieher an glänzender Scheibe, KEIN Gesicht; Friseur → Hände + Haarpartie im Close-up, Gesicht angeschnitten; Restaurant → Teller/Gläser, nicht der Gast; Reiterhof → das Pferd, nicht der Reiter. Formulierung im Prompt: "close-up on hands and the work, no face visible, cropped above shoulders". KEINE Totalen, KEINE erkennbaren Personen.
- signature_nachher = ZIEL-Zustand (der erstrebenswerte Zustand, wird ZUERST generiert, 16:9). signature_vorher = Edit-Anweisung die DIESELBE Szene festnagelt und NUR den Effekt umkehrt. Formulierung: "Edit this exact scene, keep the IDENTICAL room/furniture/objects/camera — only change: [Degradation]". Immer den aufwändigeren/volleren Zustand als Ziel und den einfacheren Zustand ableiten (wegnehmen/verschmutzen ist zuverlässiger als hinzufügen).
- DSGVO-KERNREGEL: KEINE erkennbaren Gesichter, keine identifizierbaren Personen. Grund: (1) DSGVO bei KI-Demos ohne Einwilligung, (2) Uncanny-Risiko bei KI-Gesichtern. Personen höchstens als Silhouette/von hinten/stark unscharf im Hintergrund. negativ: Markenlogos, lesbarer Text, erkennbare Gesichter.
